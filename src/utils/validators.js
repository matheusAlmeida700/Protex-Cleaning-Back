...
export const customerSchema = Joi.object({
  ...
  proactiveRestroomMonitoring: Joi.boolean().required(),
  certifiedContract: Joi.boolean().required(),
  sanitizationStatus: Joi.string().valid("done", "not done").required(),
  generalCustomerSatisfaction: Joi.number().min(1).max(10).required(),
  complaintsCount: Joi.number().min(0).required(),
